...
const SIDEBAR_SECTION_ITEMS: SidebarSectionItem[] = [
  { id: "general", labelKey: "settings.sidebar.general", icon: Settings },
  { id: "daemon", labelKey: "settings.sidebar.daemon", icon: Server, desktopOnly: true },
  { id: "appearance", labelKey: "settings.sidebar.appearance", icon: Palette },
  { id: "shortcuts", labelKey: "settings.sidebar.shortcuts", icon: Keyboard, desktopOnly: true },
  {
    id: "integrations",
  ...
  { id: "permissions", labelKey: "settings.sidebar.permissions", icon: Shield, desktopOnly: true },
  { id: "diagnostics", labelKey: "settings.sidebar.diagnostics", icon: Stethoscope },
  { id: "about", labelKey: "settings.sidebar.about", icon: Info },
];
...
const HOST_SECTION_ITEMS: HostSectionItem[] = [
  { id: "connections", labelKey: "settings.hostSection.connections", icon: Network },
  { id: "agents", labelKey: "settings.hostSection.agents", icon: Bot },
  { id: "workspaces", labelKey: "settings.hostSection.workspaces", icon: FolderGit2 },
  { id: "providers", labelKey: "settings.hostSection.providers", icon: Boxes },
  { id: "host", labelKey: "settings.hostSection.host", icon: Server },
];
...
function getSendBehaviorOptions() {
  return [
    { value: "interrupt" as const, label: i18n.t("settings.general.sendBehaviorInterrupt") },
    { value: "queue" as const, label: i18n.t("settings.general.sendBehaviorQueue") },
  ];
}

function getReleaseChannelOptions() {
  return [
    { value: "stable" as const, label: i18n.t("settings.about.releaseChannelStable") },
    { value: "beta" as const, label: i18n.t("settings.about.releaseChannelBeta") },
  ];
}

function getServiceUrlBehaviorLabel(behavior: ServiceUrlBehavior): string {
  switch (behavior) {
    case "ask":
      return i18n.t("settings.general.serviceUrlAsk");
    case "in-app":
      return i18n.t("settings.general.serviceUrlInApp");
    case "external":
      return i18n.t("settings.general.serviceUrlExternal");
  }
}
...
function ServiceUrlBehaviorMenuItem({
  ...
  return (
    <DropdownMenuItem selected={selected} onSelect={handleSelect}>
      {getServiceUrlBehaviorLabel(value)}
    </DropdownMenuItem>
  );
}
...
function GeneralSection({
  ...
  return (
    <SettingsSection title={t("settings.sidebar.general")}>
      <View style={settingsStyles.card}>
        <View style={settingsStyles.row}>
          <View style={settingsStyles.rowContent}>
            <Text style={settingsStyles.rowTitle}>{t("settings.general.defaultSendTitle")}</Text>
          ...
          <SegmentedControl
            ...
            options={getSendBehaviorOptions()}
          />
        </View>
        {isDesktopApp ? (
          <View style={ROW_WITH_BORDER_STYLE}>
            <View style={settingsStyles.rowContent}>
              <Text style={settingsStyles.rowTitle}>{t("settings.general.serviceUrlsTitle")}</Text>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger style={themeTriggerStyle}>
                <Text style={styles.themeTriggerText}>
                  {getServiceUrlBehaviorLabel(settings.serviceUrlBehavior)}
                </Text>
        ...
        <View style={ROW_WITH_BORDER_STYLE}>
          <View style={settingsStyles.rowContent}>
            <Text style={settingsStyles.rowTitle}>
              {t("settings.general.terminalScrollbackTitle")}
            </Text>
          ...
          <TextInput
            ...
            accessibilityLabel={t("settings.general.terminalScrollbackA11y")}
          />
        </View>
      </View>
    </SettingsSection>
...
function DiagnosticsSection({
  ...
  return (
    <SettingsSection title={t("settings.sidebar.diagnostics")}>
      <View style={settingsStyles.card}>
        <View style={settingsStyles.row}>
          <View style={settingsStyles.rowContent}>
            <Text style={settingsStyles.rowTitle}>{t("settings.diagnostics.testAudioTitle")}</Text>
            {playbackTestResult ? (
...
function AboutSection({ appVersion, appVersionText, isDesktopApp }: AboutSectionProps) {
  ...
  return (
    <>
      <SettingsSection title={t("settings.sidebar.about")}>
        <View style={settingsStyles.card}>
          <View style={settingsStyles.row}>
            <View style={settingsStyles.rowContent}>
              <Text style={settingsStyles.rowTitle}>{t("settings.about.appVersionTitle")}</Text>
              <Text style={settingsStyles.rowHint}>{t("settings.about.appVersionHint")}</Text>
            </View>
...
function ConnectedHostsSection({ clientVersion }: { clientVersion: string | null }) {
  ...
  return (
    <SettingsSection title={t("settings.about.connectedHostsTitle")}>
      <View style={settingsStyles.card}>
...
function HostVersionRow({
  ...
  if (!isConnected) {
    valueText = t("settings.about.hostOffline");
  ...
  return (
    <View style={rowStyle}>
      <View style={settingsStyles.rowContent}>
        ...
        {isMismatch ? (
          <Text style={settingsStyles.rowHint}>{t("settings.about.versionMismatch")}</Text>
        ) : null}
      </View>
...
function getUpdateButtonLabel(
  ...
  if (isInstalling) return i18n.t("settings.about.updateInstalling");
  if (latestVersion) {
    return i18n.t("settings.about.updateToVersion", {
      version: formatVersionWithPrefix(latestVersion),
    });
  }
  return i18n.t("settings.about.update");
}
...
function DesktopAppUpdateRow() {
  ...
  const handleInstallUpdate = useCallback(() => {
    ...
    void confirmDialog({
      title: t("settings.about.installUpdateTitle"),
      message: t("settings.about.installUpdateMessage"),
      confirmLabel: t("settings.about.installUpdateConfirm"),
      ...
      .catch((error) => {
        ...
        Alert.alert(t("settings.about.errorTitle"), t("settings.about.updateDialogError"));
      });
  ...
  return (
    <>
      <View style={ROW_WITH_BORDER_STYLE}>
        <View style={settingsStyles.rowContent}>
          <Text style={settingsStyles.rowTitle}>{t("settings.about.releaseChannelTitle")}</Text>
        ...
        <SegmentedControl
          ...
          options={getReleaseChannelOptions()}
        />
      </View>
      <View style={ROW_WITH_BORDER_STYLE}>
        <View style={settingsStyles.rowContent}>
          <Text style={settingsStyles.rowTitle}>{t("settings.about.appUpdatesTitle")}</Text>
...
function HostPicker({
  ...
  const options = useMemo<ComboboxOption[]>(() => {
    ...
    return [...hostOptions, { id: ADD_HOST_OPTION_ID, label: t("settings.host.addHost") }];
  }, [sortedHosts, t]);
  ...
  return (
    <>
      <Pressable
        ...
        onPress={handleOpen}
        accessibilityRole="button"
        accessibilityLabel={t("settings.host.switchHost")}
      ...
      >
        ...
        <Text style={sidebarStyles.pickerTriggerLabel} numberOfLines={1}>
          {activeHost?.label ?? t("settings.hostSection.host")}
        </Text>
      ...
      <Combobox
        ...
        searchable={false}
        title={t("settings.host.switchHost")}
        desktopMinWidth={240}
        open={isOpen}
  ...
  return (
    <View style={containerStyle} testID="settings-sidebar">
      ...
      <View style={sidebarStyles.list}>
        <Text style={sidebarStyles.groupLabel}>{t("settings.sidebar.groupApp")}</Text>
        {items.map((item) => (
      ...
      {hasHosts ? (
        <View style={sidebarStyles.list}>
          <Text style={sidebarStyles.groupLabel}>{t("settings.sidebar.groupHost")}</Text>
          <HostPicker
        ...
        <View style={sidebarStyles.list}>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel={t("settings.host.addHost")}
            onPress={onAddHost}
            testID="settings-add-host"
...
export default function SettingsScreen({ view }: SettingsScreenProps) {
  ...
  const detailHeader = ((): {
    ...
    if (view.kind === "project" || view.kind === "projects") {
      return { title: t("settings.sidebar.projects"), Icon: FolderGit2 };
    }
    return null;
  })();
