...
export function WorkspaceOpenInEditorButton({
  ...
  const openMutation = useMutation({
    ...
    onError: (error: unknown) => {
      toast.error(error instanceof Error ? error.message : t("workspace.openInEditor.openFailed"));
    },
  });
    ...
    <View style={styles.row}>
      <View style={styles.splitButton}>
        <Pressable
          ...
          accessibilityLabel={
            activeFileName
              ? t("workspace.openInEditor.openFileInEditor", {
                  fileName: activeFileName,
                  editor: primaryOption.label,
                })
              : t("workspace.openInEditor.openWorkspaceInEditor", { editor: primaryOption.label })
          }
        >
          {openMutation.isPending ? (
            ...
            <View style={styles.splitButtonContent}>
              {primaryOption.icon}
              {!hideLabels && (
                <Text style={styles.splitButtonText}>{t("workspace.openInEditor.open")}</Text>
              )}
            </View>
          )}
        </Pressable>
        {targets.length > 1 ? (
          <DropdownMenu>
            <DropdownMenuTrigger
              ...
              accessibilityRole="button"
              accessibilityLabel={t("workspace.openInEditor.chooseEditor")}
            >
