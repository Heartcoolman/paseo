...
function buildSyntheticDefaultRow(
  ...
  return {
    ...
    providerLabel,
    modelId: "",
    modelLabel: i18n.t("agent.providerSelection.defaultModel"),
  ...
  return {
    ...
    message:
      ...
      (entry.status === "unavailable"
        ? i18n.t("agent.providerSelection.unavailable")
        : i18n.t("agent.providerSelection.unknownError")),
  };
}
...
export function resolveSelectedModelLabel(input: {
  ...
  if (!selectedProvider) {
    return i18n.t("agent.providerSelection.selectModel");
  }
  ...
  if (!provider) {
    return input.isLoading
      ...
      : i18n.t("agent.providerSelection.selectModel");
  }
  if (provider.modelSelection.kind === "loading") {
    return i18n.t("common.state.loading");
  }
  if (provider.modelSelection.kind === "error") {
    return i18n.t("agent.providerSelection.error");
  }
...
export function resolveSubmissionReadiness(input: {
  ...
  if (!input.allowsEmptyAutoSubmit && !input.text.trim()) {
    return { ok: false, reason: i18n.t("agent.readiness.promptRequired") };
  }
  if (input.providerCount === 0) {
    return { ok: false, reason: i18n.t("agent.readiness.noProviders") };
  }
  if (!(input.autoSubmitConfig?.provider ?? input.selection.provider)) {
    return { ok: false, reason: i18n.t("agent.readiness.selectModel") };
  }
  if (input.selection.isModelLoading) {
    return { ok: false, reason: i18n.t("agent.readiness.modelDefaultsLoading") };
  }
  ...
  if (!hasSelectedModel && input.selection.availableModels.length > 0) {
    return { ok: false, reason: i18n.t("agent.readiness.noModelForProvider") };
  }
  if (!input.workspaceDirectory) {
    return { ok: false, reason: i18n.t("agent.readiness.workspaceNotFound") };
  }
  if (!input.hasClient) {
    return { ok: false, reason: i18n.t("agent.readiness.hostNotConnected") };
  }
  return { ok: true };
}
