...
function useTerminalPanelDescriptor(
        ...
        if (!client || !workspaceDirectory) {
          throw new Error(
            workspaceAuthority.ok
              ? t("workspace.panel.executionDirNotFoundError")
  ...
  return {
    label: trimNonEmpty(terminal?.title ?? terminal?.name ?? null) ?? t("workspace.tab.terminal"),
    subtitle: t("workspace.tab.terminal"),
    titleState: "ready",
    icon: Terminal,
    statusBucket: null,
  };
  ...
  if (!workspaceDirectory) {
    return (
      <View style={CENTERED_PADDED_STYLE}>
        <Text>
          {workspaceAuthority.ok
            ? t("workspace.panel.executionDirNotFound")
            : workspaceAuthority.message}
        </Text>
      </View>
    );
