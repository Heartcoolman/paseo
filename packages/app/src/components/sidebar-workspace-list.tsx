  ...
  const handleOpenInNewWindow = useCallback(() => {
    ...
    void getDesktopHost()
      ...
      ?.catch((error) => {
        ...
        toast.error(t("common.sidebar.openNewWindowFailed"));
      });
  }, [projectPath, toast, t]);
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        ...
        accessibilityLabel={t("common.sidebar.projectActions")}
      ...
      <DropdownMenuContent align="end" width={220}>
        {canOpenProjectSettings ? (
          ...
          >
            {t("common.sidebar.openProjectSettings")}
          </DropdownMenuItem>
        ) : null}
        {canOpenInNewWindow ? (
          <DropdownMenuItem
          ...
          >
            {t("common.sidebar.openInNewWindow")}
          </DropdownMenuItem>
        ) : null}
        <DropdownMenuItem
          ...
          pendingLabel={t("common.sidebar.removing")}
          onSelect={onRemoveProject}
        >
          {t("common.sidebar.removeProject")}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
function WorkspaceRowRightGroup({
  ...
  return (
    <>
      {isCreating ? (
        <Text style={styles.workspaceCreatingText}>{t("common.sidebar.creating")}</Text>
      ) : null}
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        ...
        accessibilityRole={platformIsWeb ? undefined : "button"}
        accessibilityLabel={t("common.sidebar.workspaceActions")}
      ...
      <DropdownMenuContent align="end" width={260}>
        ...
        {onCopyBranchName ? (
          ...
          >
            {t("common.sidebar.copyBranchName")}
          </DropdownMenuItem>
        ) : null}
        {onRename ? (
          <DropdownMenuItem
        ...
        <DropdownMenuItem
          ...
          onSelect={onArchive}
        >
          {archiveLabel ?? t("common.sidebar.archive")}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
      ...
      <Tooltip delayDuration={0} enabledOnDesktop enabledOnMobile={false}>
        <TooltipTrigger asChild disabled={!visible}>
          <Pressable
            ...
            disabled={loading}
            accessibilityRole={platformIsWeb ? undefined : "button"}
            accessibilityLabel={t("common.sidebar.createWorkspaceFor", { name: displayName })}
            testID={testID}
          >
            {({ hovered, pressed }) =>
              loading ? (
        ...
        <TooltipContent side="bottom" align="center" offset={8}>
          <View style={styles.projectActionTooltipRow}>
            <Text style={styles.projectActionTooltipText}>{t("common.sidebar.newWorkspace")}</Text>
...
function WorkspaceRowWithMenu({
  ...
  const archiveWorktreeAfterConfirmation = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : t("common.sidebar.workspacePathUnavailable"),
      );
      return;
    }

    if (!archiveDirectory) {
      toast.error(t("common.sidebar.workspacePathUnavailable"));
      return;
    }
    ...
    void archiveWorktree({
      ...
      const message =
        error instanceof Error ? error.message : t("common.sidebar.archiveWorktreeFailed");
      toast.error(message);
    });
  }, [archiveWorktree, isArchiving, redirectAfterArchive, toast, workspace, t]);
  ...
  const hideWorkspaceAfterConfirmation = useCallback(async () => {
    ...
    const confirmed = await confirmDialog({
      title: t("common.sidebar.hideWorkspaceTitle"),
      message: t("common.sidebar.hideWorkspaceMessage", { name: workspace.name }),
      confirmLabel: t("common.sidebar.hide"),
    ...
    if (!client) {
      toast.error(t("common.sidebar.hostNotConnected"));
      return;
    }
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("common.sidebar.hideWorkspaceFailed"));
    } finally {
      setIsArchivingWorkspace(false);
    }
  }, [isArchivingWorkspace, redirectAfterArchive, toast, workspace, t]);
  ...
  const handleCopyPath = useCallback(() => {
    ...
    toast.copied(t("common.sidebar.pathCopied"));
  }, [toast, workspace.workspaceDirectory, workspace.workspaceId, t]);

  const handleCopyBranchName = useCallback(() => {
    void Clipboard.setStringAsync(workspace.name);
    toast.copied(t("common.sidebar.branchNameCopied"));
  ...
  const renameMutation = useMutation({
    mutationFn: async (branch: string) => {
      const client = getHostRuntimeStore().getClient(workspace.serverId);
      if (!client) {
        throw new Error(i18n.t("common.sidebar.hostNotConnected"));
      }
      ...
      if (!payload.success || payload.error) {
        throw new Error(payload.error?.message ?? i18n.t("common.sidebar.renameBranchFailed"));
      }
      return { targetCwd };
    },
  ...
  const validateRenameSlug = useCallback(
    (value: string): string | null => {
      ...
      return result.error ?? t("common.sidebar.invalidBranchName");
    },
    [t],
  );
  ...
  const handleMarkAsRead = useCallback(() => {
    void clearAttention().catch((error) => {
      toast.error(error instanceof Error ? error.message : t("common.sidebar.markAsReadFailed"));
    });
  ...
  return (
    <>
      <WorkspaceRowInner
        ...
        menuController={null}
        archiveLabel={
          isWorktree ? t("common.sidebar.archiveWorktree") : t("common.sidebar.hideFromSidebar")
        }
        archiveStatus={getWorkspaceArchiveStatus(isWorktree, archiveStatus, isArchivingWorkspace)}
        archivePendingLabel={
          isWorktree ? t("common.sidebar.archiving") : t("common.sidebar.hiding")
        }
      ...
      <AdaptiveRenameModal
        ...
        title={t("common.sidebar.renameWorkspace")}
        initialValue={workspace.name}
        placeholder="branch-name"
        submitLabel={t("common.action.rename")}
    ...
    void (async () => {
      ...
      void (async () => {
        try {
          ...
          toast.error(
            error instanceof Error ? error.message : t("common.sidebar.hideWorkspaceFailed"),
          );
        } finally {
          setIsArchivingWorkspace(false);
        }
    ...
    <>
      ...
      >
        <ContextMenuItem
          ...
          pendingLabel={t("common.sidebar.hiding")}
          destructive
          onSelect={handleArchiveWorkspace}
        >
          {t("common.sidebar.hideFromSidebar")}
        </ContextMenuItem>
      </ContextMenuContent>
    </>
  );
  ...
  const handleRemoveProject = useCallback(() => {
    ...
    void (async () => {
      const confirmed = await confirmDialog({
        ...
        message: t("common.sidebar.removeProjectMessage", { name: displayName }),
      ...
      void archiveWorkspacesOptimistically({
        ...
        if (failures.length > 0) {
          toast.error(t("common.sidebar.removeWorkspacesFailed"));
        }
        setIsRemovingProject(false);
        return;
      });
    })();
  }, [isRemovingProject, serverId, displayName, toast, project.workspaces, t]);
    ...
    <>
      {projects.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyTitle}>{t("common.sidebar.noProjects")}</Text>
          <Text style={styles.emptyText}>{t("common.sidebar.addProjectToStart")}</Text>
          <Button variant="ghost" size="sm" leftIcon={Plus} onPress={onAddProject}>
            {t("common.sidebar.addProject")}
          </Button>
        </View>
      ) : (
        <DraggableList
