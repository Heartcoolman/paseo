...
export function PairLinkModal({ visible, onClose, onCancel, onSaved }: PairLinkModalProps) {
  ...
  const header = useMemo<SheetHeader>(() => ({ title: t("pairing.link.modalTitle") }), [t]);
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!raw) {
      setErrorMessage(t("pairing.link.pasteRequired"));
      return;
    }
    if (!raw.includes("#offer=")) {
      setErrorMessage(t("pairing.link.mustIncludeOffer"));
      return;
    }
    ...
    const parsedOffer = (() => {
      try {
        ...
        if (!encoded) {
          throw new Error(t("pairing.link.offerEmpty"));
        }
        const payload = decodeOfferFragmentPayload(encoded);
        return ConnectionOfferSchema.parse(payload);
      } catch (error) {
        const message = error instanceof Error ? error.message : t("pairing.link.invalidLink");
        setErrorMessage(message);
        if (!isMobile) {
          Alert.alert(t("pairing.link.pairingFailed"), message);
        }
        return null;
      }
    })();
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t("pairing.link.unablePair");
      setErrorMessage(message);
      if (!isMobile) {
        Alert.alert(t("pairing.link.pairingFailed"), message);
      }
  ...
  }, [daemons, handleClose, isMobile, isSaving, onSaved, upsertDaemonFromOfferUrl, t]);
  ...
  return (
    ...
    >
      ...
      <View style={styles.field}>
        <Text style={styles.label}>{t("pairing.link.fieldLabel")}</Text>
        <AdaptiveTextInput
          ref={inputRef}
      ...
      <View style={styles.actions}>
        ...
        <Button
          ...
          accessibilityRole="button"
          accessibilityLabel={t("pairing.link.pair")}
          leftIcon={pairIcon}
        >
          {isSaving ? t("pairing.link.pairing") : t("pairing.link.pair")}
        </Button>
      </View>
    </AdaptiveModalSheet>
  );
