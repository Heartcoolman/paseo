...
export function useImageAttachmentPicker(): UseImageAttachmentPickerResult {
  ...
  const ensurePermission = useCallback(async () => {
    ...
    if (!currentPermission?.granted) {
      Alert.alert(
        t("common.imagePicker.permissionTitle"),
        t("common.imagePicker.permissionMessage"),
      );
      return false;
    }
  ...
  const pickImages = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(t("common.imagePicker.errorTitle"), t("common.imagePicker.selectFailed"));
      return null;
    } finally {
