...
export function useFileExplorerActions(params: { serverId: string } & FileExplorerWorkspaceScope) {
    ...
    async (
      ...
      if (!normalizedWorkspaceRoot) {
        updateExplorerState((state) => ({
          ...state,
          isLoading: false,
          lastError: t("common.fileExplorer.workspaceUnavailable"),
          pendingRequest: null,
        }));
        return false;
      }
      ...
      if (!client) {
        updateExplorerState((state) => ({
          ...state,
          isLoading: false,
          lastError: t("common.host.notConnected"),
          pendingRequest: null,
        }));
        return false;
      }
      ...
      try {
        ...
        updateExplorerState((state) => ({
          ...
          isLoading: false,
          lastError:
            error instanceof Error ? error.message : t("common.fileExplorer.failedToListDirectory"),
          pendingRequest: null,
        }));
        return false;
      }
        ...
        updateExplorerState((state) => ({
          ...
          lastError:
            error instanceof Error
              ? error.message
              : t("common.fileExplorer.failedToLoadFilePreview"),
          pendingRequest: null,
        }));
      }
    },
  ...
  const requestFileDownloadToken = useCallback(
    async (path: string) => {
      if (!normalizedWorkspaceRoot) {
        throw new Error(t("common.fileExplorer.workspaceUnavailable"));
      }
      if (!client) {
        throw new Error(t("common.host.notConnected"));
      }
