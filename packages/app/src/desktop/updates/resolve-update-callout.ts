...
export function resolveUpdateCalloutDescriptor(
  ...
  if (isInstalling) {
    title = i18n.t("desktop.update.installingTitle");
    body = { kind: "installing" };
  } else if (isError) {
    title = i18n.t("desktop.update.failedTitle");
    body = { kind: "error", message: input.errorMessage ?? i18n.t("desktop.update.genericError") };
  } else {
    title = i18n.t("desktop.update.availableTitle");
    body = { kind: "available", versionLabel: formatVersionLabel(latestVersion) };
  }

  const actions: UpdateCalloutActionDescriptor[] = [
    { role: "changelog", label: i18n.t("desktop.update.whatsNew") },
  ];
  if (isError) {
    ...
    actions.push({
      role: "install",
      label: isInstalling
        ? i18n.t("desktop.update.installingAction")
        : i18n.t("desktop.update.installAndRestart"),
      variant: "primary",
